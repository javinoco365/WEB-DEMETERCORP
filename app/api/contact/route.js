...
export async function POST(req){
  try{
    ...
    const name=clean(body.name,120), company=clean(body.company,160), email=clean(body.email,200), subject=clean(body.subject,160), message=clean(body.message,5000)
    ...
    const html=`<h2>Nueva consulta desde demetercorp.es</h2><p><strong>Nombre:</strong> ${esc(name)}</p><p><strong>Empresa:</strong> ${esc(company)||'—'}</p><p><strong>Email:</strong> ${esc(email)}</p><p><strong>Motivo:</strong> ${esc(subject)}</p><p><strong>Mensaje:</strong></p><p>${esc(message).replace(/\n/g,'<br>')}</p>`
    const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[to],reply_to:email,subject:`Web Demeter Corp · ${subject}`,html})})
